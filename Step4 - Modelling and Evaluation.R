...
#Divide the climate data in to training and testing datasets
...
library(rpart.plot)
set.seed(12345)
data_rand <- climateData.norm[order(runif(636)), ] #creating  a new dataframe 
...
head(climateData.norm$temperature) # returns the first 5 rows
head(data_rand$temperature) # we check the order of both dataframes are different !

#splitting the dataset
data_train <- data_rand[1:570, ]
data_test  <- data_rand[571:636, ]
